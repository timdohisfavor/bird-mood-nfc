...
async function createDetailShareCanvas(bird) {
  ...
  items.forEach(function(item, index) {
    ...
    ctx.save();
    ctx.shadowColor = "rgba(31, 46, 36, 0.06)";
    ctx.shadowBlur = 16;
    ctx.shadowOffsetY = 6;
    ...
    ctx.fill();
    ctx.restore();
    ctx.strokeStyle = "rgba(130,108,64,0.18)";
    ctx.lineWidth = 2;
    ctx.stroke();
    ctx.textAlign = "center";
    ctx.fillStyle = "#66756b";
    ...
    if (item[0] === "状态") {
      ...
      var pillW = Math.max(96, ctx.measureText(statusText).width + 42);
      var pillX = x + (cardW - pillW) / 2;
      var pillY = y + 66;
      ctx.fillStyle = "rgba(224, 221, 175, 0.44)";
      drawRoundRect(ctx, pillX, pillY, pillW, 44, 22);
      ctx.fill();
      ctx.fillStyle = "#123225";
      ctx.font = "900 30px sans-serif";
      ctx.fillText(statusText, x + cardW / 2, y + 98);
    } else {
      ctx.fillStyle = "#123225";
      ctx.font = "900 32px sans-serif";
  ...
  drawSoundOrb(ctx, width / 2, callY + 250);
